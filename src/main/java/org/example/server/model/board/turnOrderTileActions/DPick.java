...
public class DPick {
    public Card execute(Match match, Player player, int id) throws NoDrawableCardException, InvalidCardException {
        ...
        if ( !isValidForDrawing(board.getBottomRow(), player)) {
            throw new NoDrawableCardException("There are no more cards drawable, you pass the turn...");
        }
        ...
        Card card = board.getBottomRow().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new InvalidCardException("Invalid ID card"));
        ...
        if (card.isBuilding()) {
            ...
            if (player.getFood() + player.getDiscountOnBuilding() < buildingCard.getFoodCost()) {
                throw new InvalidCardException("Player doesn't have enough food to buy this building card");
            } else {
                return buildingCard;
            }
        }
